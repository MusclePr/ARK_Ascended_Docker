...
| :--- | :--- | :--- | :--- |
| `cluster/maintenance.request` | `manager update` や `stop` 実行時 (`helper.sh`) | メンテナンスモード終了時、またはスクリプト終了時 | クラスター全体のメンテナンス要求シグナル。このファイルが存在すると、各サーバーは停止するか起動を待機します。 |
| `cluster/maintenance.lock` | 管理者による手動作成を想定 | メンテナンスモード終了時 (`helper.sh`) | 強制的なメンテナンスロック。サーバーの自動起動を確実に阻止するために使用されます。 |
| `server_<PORT>/waiting.flag` | `start.sh` (メンテナンス検知時) | メンテナンス終了後の再開時 | 特定のサーバーポート (`<PORT>`) がメンテナンス待機状態にあることを示します。 |
| `cluster/master_ready.flag` | MasterサーバーのRCON接続確認後 (`manager.sh`) | メンテナンス開始前、またはサーバー起動時 | Masterサーバーが完全に起動し、接続可能であることを他のクラスタノードに通知します。各ノードはこのファイルを検知してから起動を完了します。 |
| `cluster/master.lock/` | `start.sh` (Master起動時) | Masterサーバー停止時 | Masterサーバーの多重起動を防ぐためのロックディレクトリ。 |
| `cluster/updating.lock` | `manager.sh` (Update開始時) | Update完了または失敗時 | SteamCMDによるアップデート処理が進行中であることを示し、重複実行を防ぎます。 |
...
| ファイル名 | 作成タイミング (作成元) | 削除タイミング (生存期間) | 目的・用途 |
| :--- | :--- | :--- | :--- |
| `cluster/session_name.lock/` | セッション名生成/取得時 (`helper.sh`) | 生成/取得処理の完了直後 | `GameUserSettings.ini` のセッション名設定時の競合を防ぐための排他制御用ディレクトリ。 |
...
| `server_<PORT>/version` | サーバー起動時 (`start.sh`) | なし | 外部監視用に、最後に起動したサーバーのバージョン（"89.48" 等）をテキストで保持します。 |

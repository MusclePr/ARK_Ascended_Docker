...
ALLOWED_FILE="/opt/arkserver/.cluster_update.allowed"
MASTER_LOCK_DIR="/opt/arkserver/.cluster_master.lock"
MASTER_LOCK_OWNER_FILE="${MASTER_LOCK_DIR}/owner"
...
on_shutdown_signal() {
    ...
    manager stop --saveworld >/dev/null 2>&1 || true
    exit 0
}
...
wait_for_valid_installation() {
    ...
    while [[ ! -f "/opt/arkserver/ShooterGame/Binaries/Win64/ArkAscendedServer.exe" ]] || \
          [[ -f "/opt/arkserver/.cluster_updating" ]] || \
          [[ -f "$REQUEST_FILE" ]] || \
          [[ -f "$LOCK_FILE" ]]; do
        if [[ -f "$REQUEST_FILE" || -f "$LOCK_FILE" ]]; then
            touch "$WAITING_FILE"
...
if [[ "$is_master" == true ]]; then
    ...
    acquire_master_lock_or_exit
    LogInfo "AUTO_UPDATE_ENABLED=true: acting as update master"
    LogInfo "Running update check (and update if needed) via manager"
    manager update --no-warn
else
    LogInfo "AUTO_UPDATE_ENABLED!=true: acting as update slave"
    wait_for_master_allowed
fi
...
if [ "${AUTO_UPDATE_ENABLED,,}" = true ]; then
    ...
    LogInfo "Adding cronjob for cluster maintenance check"
    echo "* * * * * bash /usr/local/bin/manager check_maintenance" >> "$CRONTAB_FILE"
fi

...
- CLUSTER_ID 設定の追加
- ダウンロード失敗時のリトライ機能の追加
- SLAVE_PORTS 設定の追加。共有しているプログラムのアップデート時に、安全にダウンさせておきたい他のサーバーをポート番号で指定します。
- SERVERGAMELOG 設定の追加
...
#### プログラムおよびMODを共有する場合
プログラムやMODを共有してディスクスペースを節約したい場合は、[compose.yml](./compose.yml) を参照してください。

#### クラスター更新の master / slave
- 更新（サーバーバイナリ/共有MOD）の実行コンテナは **1つ** に限定します。
- `AUTO_UPDATE_ENABLED=true` のコンテナが **master** です。起動時に `manager update` を実行し、必要ならクラスタメンテナンスに入り更新します。
- `AUTO_UPDATE_ENABLED=false` のコンテナは **slave** です。master が更新チェックを完了するまで起動を待機します（SIGINT/SIGTERM で停止）。
- `SLAVE_PORTS` を **master** コンテナに、**slave** が使用するポート番号をカンマで連結して設定してください。
- master が複数起動した場合は、共有ボリューム上のロック（`/opt/arkserver/.cluster_master.lock`）で検知し、後発の master は終了します。

#### ロック/フラグの手動解除（kill -9 等で残った場合）
1) いったん全コンテナを停止します（例: `docker compose down`）
2) 共有ボリューム内のロック/フラグを削除します（コンテナ内で実行する例）
